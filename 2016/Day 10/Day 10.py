...
while True:
    ...
    if 'value' in line:
        ...
        b, l, h = [int(s) for s in line.split() if s.isnumeric()]
        
        initialize_bot(b)
        if not 'low to output' in line:
            initialize_bot(l)
            bots[b].low_bot = bots[l]
        else:
            initialize_output(l)
            bots[b].low_bot = outputs[l]
        if not 'high to output' in line:
            initialize_bot(h)
...
part1 = [b.num for b in bots.values() if b.low == 17 and b.high == 61][0]
part2 = prod([v.output for k, v in outputs.items() if k <= 2])
